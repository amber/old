...
sb.extend(sb.ObjectStream.prototype, {
    ...
    writeFixedFormat: function (object) {
        ...
        case 11: // ByteArray
            this.stream.uint32(object.value.length);
            this.stream.arrayBuffer(object.value.buffer, false, object.value.length);
            break;
        case 13: // Bitmap
            this.stream.uint32(object.value.length / 4);
            this.stream.arrayBuffer(object.value.buffer);
            break;
        case 20: // Array
    ...
    formats: {
        ...
        34: {
            write: function (object, table, hint) {
                var bytes = object.getContext('2d').getImageData(0, 0, object.width, object.height).data;
                var tmp;
                ...
                var bitmap = new Uint32Array(bytes.buffer);
                var stream = new sb.WriteStream();
                ...
                while (i < size) {
                    value = bitmap[i];
                    check = value & 255;
                    same = ((value >> 8 & 255) === check) &&
                        ((value >> 16 & 255) === check) &&
                        ((value >> 24 & 255) === check);
                    runIndex = i;
                    while (runIndex < size && value === bitmap[runIndex + 1]) {
                        runIndex++;
                    }
                    if (runIndex > i) {
                        if (same) {
                            encodeInt((runIndex - i + 1) * 4 + 1);
                            stream.uint8(check);
                        } else {
                            encodeInt((runIndex - i + 1) * 4 + 2);
                            stream.uint32(value);
                        }
                        i = runIndex + 1;
                    } else {
                        if (same) {
                            ...
                            while (runIndex < size && bitmap[runIndex] !== bitmap[runIndex + 1]) {
                                runIndex++;
                            }
                            if (runIndex + 1 === size) {
                                runIndex++;
                            }
                            encodeInt((runIndex - i) * 4 + 3);
                            for (var j = i; j < runIndex; j++) {
                                stream.uint32(bitmap[j]);
                            }
                            i = runIndex;
                        }
                    }
                ...
                return {
                    ...
                    depth: 32,
                    offset: null,
                    bitmap: this.createObject(table, stream.bytes(), 11)
                }
            }
        },
        124: {
