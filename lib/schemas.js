...
Project.query = function (query, sort, offset, length, fields, cb) {
    Project.find(query).sort(sort).skip(offset).limit(length).exec(function (err, result) {
...
ForumSchema.virtual('name').get(function () {
    return this._id;
}).set(function (name) {
    this._id = name;
});
