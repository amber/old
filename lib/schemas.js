    ...
    Async.each(Object.keys(fields), function (field, cb) {
        ...
        if (Array.isArray(model[field])) {
            ...
            Async.each(c, function (step, cb) {
                ...
                mongoose.model(model.schema.paths[field].options.type[0].ref).findById(step.value, function (err, obj) {
                    step.value = {};
        ...
        } else if (!equals(model[field], model.before[field])) {
...
function updateHooks(schema, options) {
    ...
    schema.post('save', function () {
        ...
        if (myWatchers) {
            for (var i = 0; i < myWatchers.length; i++) {
                if (matchesQuery(this, myWatchers[i].$query)) {
                    var cb = myWatchers[i].$cb;
                    modelChanges(self, myWatchers[i].fields, function (changes) {
                        if (changes) {
                            cb(self._id, changes);
                        }
                    });
